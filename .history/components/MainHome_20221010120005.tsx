...
const MainHome = () => {
  ...
  return (
    <div className="flex flex-col text-white  min-h-screen w-full z-10 py-10 px-4 md:px-10 md:py-16">
      <div>
        {/* main */}
        <div className="z-10 w-full bg-blue-200 flex flex-col">
          ...
          <div className="">
            {HeroImg.map((hero, index) => (
              <div key={hero.id}>
                {index === currentCategory && <div>hadnkaj</div>}
              </div>
            ))}
          </div>
        </div>
        {/* Photography */}
        <div className="py-96">j;nv</div>
        <div className="py-96">j;nv</div>
        {/* Videography */}
        <div></div>
        {/* Events */}
