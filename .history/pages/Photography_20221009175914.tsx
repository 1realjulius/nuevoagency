...
const Photography = () => {
  ...
  const newX = useSpring(200, { stiffness: 100, damping: 10 });
  const newY = useSpring(200, { stiffness: 100, damping: 10 });

  const rotateX = useTransform(newX, [0, 400], [-45, 45]);
  const rotateY = useTransform(newY, [0, 400], [-45, 45]);

  function handleMouse(event: any) {
    const rect = event.currentTarget.getBoundingClientRect();

    newX.set(event.clientX - rect.left);
    newY.set(event.clientY - rect.top);
  }
  ...
  const variants = {
    ...
    btnHover: {
      newX: mousePosition.x - 10,
      newY: mousePosition.y - 10,
      transition: { ease: "easeOut", duration: 0.5 },
    },
  };
